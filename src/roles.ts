...
export const BUILTIN_ROLES: Record<string, AgentRole> = {
	...
	reviewer: {
		...
		writeAccess: "none",
		prompt:
			"Work read-only. Review the specified change for correctness, regressions, security, data loss, race conditions, and missing tests. Lead with actionable findings ordered by severity, cite exact files and lines, and avoid style-only commentary. Do not modify files.",
	},
	researcher: {
		profile: "agy",
...
export function buildRoleAssignment(
	...
	return [
		...
		task,
		"</assignment>",
		...(taskMarker ? [`[herdr-task-marker:${taskMarker}]`] : []),
	].join("\n");
}
...
Use background mode for independent parallel work and foreground mode when the current turn needs one result. Keep every returned agent_id. Retrieve background work with herdr_get_agent_result, use herdr_list_agents if an id is lost, redirect live work with herdr_steer_agent, and close owned panes with herdr_stop_agent when they are no longer needed. Write-capable roles are serialized per working directory; use separate Git worktrees for parallel writers. Review peer output critically before acting on it.
